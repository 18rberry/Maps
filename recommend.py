...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    find_predictor_values = {}
    for feature in feature_fns:
        predictor, r_squared_value = find_predictor(user, reviewed, feature)
        find_predictor_values[predictor] = r_squared_value

    return max(find_predictor_values.keys(), key = lambda x: find_predictor_values.get(x))
...
def rate_all(user, restaurants, feature_fns):
    ...
    # BEGIN Question 9
    "*** YOUR CODE HERE ***"
    ratings_for_restaurants = {}
    for restaurant in restaurants:
        name = restaurant_name(restaurant)
        if restaurant in reviewed:
            ratings_for_restaurants[name] = user_rating(user, name)
        else:
            ratings_for_restaurants[name] = predictor(restaurant)
    return ratings_for_restaurants
    # END Question 9
...
def search(query, restaurants):
    ...
    # BEGIN Question 10
    "*** YOUR CODE HERE ***"
    list_of_restaurants = [restaurant for restaurant in restaurants if query in restaurant_categories(restaurant)]
